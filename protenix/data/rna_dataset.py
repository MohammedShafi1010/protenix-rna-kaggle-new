...
class SimpleRNADataset(Dataset):
    def __init__(
        ...
        # print("cwd:", os.getcwd())
        # # 2. Point at the actual casp16 directory
        # #    (replace this with the absolute path you see in your VS Code explorer)
        # casp_vfold_result_dir = '/home/ubuntu/shafi_workspace/Protenix-RNA-Kaggle/data/casp16'
        # # 3. List everything in that folder
        # print("dir listing:", os.listdir(casp_vfold_result_dir))
        # # 4. Glob just the .pdb files
        # pattern = os.path.join(casp_vfold_result_dir, '*.pdb')
        # print("pdbs found:", glob.glob(pattern))
        # self.name_to_xyz = {}
...
###############################################
        ...
        # with open(self.input_json_path, 'r') as f:
        #     self.inputs = json.load(f)
        # print('First sample:', self.inputs[0])
        # # Absolute path to CASP16 results (flat directory of .pdb files)
        # casp_vfold_result_dir = '/home/ubuntu/shafi_workspace/Protenix-RNA-Kaggle/data/casp16'
        # print('cwd:', os.getcwd())
        # print('CASP16 dir listing:', os.listdir(casp_vfold_result_dir))
        # self.name_to_xyz = {}
        # for sample in self.inputs:
        #     name = sample['name']
        #     pdb_file = os.path.join(casp_vfold_result_dir, f"{name}.pdb")
        #     # Verify the single PDB file exists
        #     if not os.path.isfile(pdb_file):
        #         raise FileNotFoundError(f"PDB file not found: {pdb_file}")
        #     print(f"Parsing {pdb_file}")
        #     # Parse the PDB into coordinates (L, 3)
        #     xyz, _ = self.parse_pdb_to_xyz(pdb_file)
        #     self.name_to_xyz[name] = xyz
        #     print(f"{name} coords shape: {xyz.shape}")
        # # Store crop size for later
        # self.crop_size = crop_size
        # Store config
        self.input_json_path = input_json_path
        self.dump_dir = dump_dir
        ...
        for sample in self.inputs:
            ...
            # Parse the PDB into coordinates (L, 3)
            xyz, _ = self.parse_pdb_to_xyz(pdb_file)
            # Wrap into shape (1, L, 3) to match downstream expectations
            coords = np.expand_dims(xyz, axis=0)
            self.name_to_xyz[name] = coords
